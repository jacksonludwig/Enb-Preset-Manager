...
int LS <string UIName="-------------------LOCAL SATURATION";  string UIWidget="Spinner";  int UIMin=0;  int UIMax=0;> = {0};
    float4 fSaturation_E < string UIName="COLOR: Local Saturation [E]-";  string UIWidget="Spinner";  float UIMin=0.0;  float UIMax=10.0; > = {1.0, 1.0, 1.0, 1.0};
    float fSaturation_I < string UIName="COLOR: Local Saturation [I]-";  string UIWidget="Spinner";  float UIMin=0.0;  float UIMax=10.0; > = {1.0};
    float fSaturation_D < string UIName="COLOR: Local Saturation [D]-";  string UIWidget="Spinner";  float UIMin=0.0;  float UIMax=10.0; > = {1.0};
...
float3 msLocalSaturation(float3 color, float eBlur) {
/// Time and Location interpolators
  float fSaturation = TODEID2(Params01[3], fSaturation_E, fSaturation_I, fSaturation_D);

    float3 avgSceneLuma = AvgLuma(color.rgb).w;                                      /// Average the scene color luminance
    color.rgb           = lerp(avgSceneLuma, color.rgb, fSaturation / (eBlur + 1));  /// Apply Luminance based Saturation with Localized adjustments
